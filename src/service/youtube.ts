...
class Youtube {
	async getAudio(videoID: string) {
		let info = await this.getInfoVideo(videoID);
		let audioFormats = ytdl.filterFormats(info.formats, "audioonly");
		let audio = audioFormats.reduce(
			(previousValue: ytdl.videoFormat, currentValue: ytdl.videoFormat) => {
				if (previousValue.audioBitrate !== undefined && currentValue.audioBitrate !== undefined) {
					return previousValue.audioBitrate > currentValue.audioBitrate
						? previousValue
						: currentValue;
				}
